...
from amp_rsl_rl.dha_utils import compute_ms_observations, compute_ms_observations_ideal
    ...
    def act(self, obs: torch.Tensor, critic_obs: torch.Tensor) -> torch.Tensor:
        ...
        if type(self.actor_critic).__name__ == "ActorCriticMoESymm" or type(self.actor_critic).__name__ == "ActorCriticSymm":
            if self.is_ideal:
                ...
                self.transition.values = self.actor_critic.evaluate(compute_ms_observations(critic_obs, self.joint_order_for_morphosymm, self.amp_joint_names)).detach()
        else:
    ...
    def compute_returns(self, last_critic_obs: torch.Tensor) -> None:
        ...
        if type(self.actor_critic).__name__ == "ActorCriticMoESymm" or type(self.actor_critic).__name__ == "ActorCriticSymm":
            if self.is_ideal:
                ...
                last_values = self.actor_critic.evaluate(compute_ms_observations(last_critic_obs, self.joint_order_for_morphosymm, self.amp_joint_names)).detach()
        else:
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(
            ...
            if type(self.actor_critic).__name__ == "ActorCriticMoESymm" or type(self.actor_critic).__name__ == "ActorCriticSymm":
                if self.is_ideal:
                    ...
                    obs_batch = compute_ms_observations(obs_batch, self.joint_order_for_morphosymm, self.amp_joint_names)
                    critic_obs_batch = compute_ms_observations(critic_obs_batch, self.joint_order_for_morphosymm, self.amp_joint_names)
